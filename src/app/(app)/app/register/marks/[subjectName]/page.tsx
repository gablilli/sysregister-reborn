    ...
    return (
        <div className="mx-auto max-w-3xl p-4">
            <div className="mb-2 mt-2">
                <p className="text-3xl font-semibold ph-censor-text">{decodeURIComponent(subjectName as string)}</p>
            </div>
                ...
                <CarouselContent>
                    {periods.map((period, index) => (
                        <CarouselItem key={JSON.stringify(period)}>
                            <div className="p-4">
                                <p className="font-semibold text-center text-lg ph-censor-text">{period}</p>
...
function TrendGraph({ marks }: { marks: GradeType[] }) {
    ...
    return (
        <div className="relative ph-no-capture p-4 pb-2 px-0 rounded-xl overflow-hidden">
            <div className="bg-secondary -z-10 opacity-25 absolute top-0 bottom-0 left-0 right-0" />
        ...
        <Tabs defaultValue={periods[0]} className="min-h-[80svh]" >
            <div className="sticky top-0 z-10 shadow-xl pt-4 pb-2 bg-background">
                ...
                <TabsList className="grid w-full" style={{ gridTemplateColumns: `repeat(${periods.length}, minmax(0, 1fr))` }}>
                    {periods.map((period, index) => (
                        <TabsTrigger key={index} value={period} className="ph-censor-text">{period}</TabsTrigger>
                    ))}
                </TabsList>
            </div>
            {periods.map((period, index) => (
        ...
        <div ref={parent} key={mark.evtId} onClick={async () => {
            ...
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                    ...
                    <div className="">
                        <p className="font-semibold ph-censor-text">{mark.componentDesc ? mark.componentDesc : "Voto di prova"}</p>
                        <p className="opacity-60 text-sm ph-censor-text">{mark.evtDate}</p>
                    </div>
                </div>
            ...
            {notes != null && (
                <div className="flex items-center mt-3 relative p-4 rounded-lg overflow-hidden">
                    ...
                    {!notes ? <span className="italic font-semibold ph-censor-text">Il docente non ha inserito note per la famiglia.</span> : <span className="whitespace-pre-wrap ph-censor-text">{notes}</span>}
                </div>
            )}
        </div>
    )
